    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Make changes to your lesson details here. Click save when you're
            done.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
...
const LessonGrid: FC<{
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    {lessons.map((lesson) => {
      const { icon: StatusIcon, className: statusClassName } = getStatusIcon(
        lesson.status,
      );
      return (
        <Card
        ...
        >
          <CardHeader className="relative">
            ...
            <div className="flex items-center gap-2">
              <Badge
                variant="secondary"
                className={cn(skillStyles[lesson.skill as Skill])}
              >
                {lesson.skill}
              </Badge>
...
const MyLessonsView: FC<MyLessonsViewProps> = ({
  lessons,
  setLessons,
  setActiveViewState,
}) => {
  ...
  const [skillFilters, setSkillFilters] =
    useState<Record<Skill, boolean>>(initialSkillFilters);
  const [tempSkillFilters, setTempSkillFilters] =
    useState<Record<Skill, boolean>>(initialSkillFilters);
  const [isSkillFilterOpen, setIsSkillFilterOpen] = useState(false);

  const initialLevelFilters = Object.keys(levelMapping).reduce(
    (acc, key) => {
      acc[key] = true;
      return acc;
    },
  ...
  const [levelFilters, setLevelFilters] =
    useState<Record<string, boolean>>(initialLevelFilters);
  const [tempLevelFilters, setTempLevelFilters] =
    useState<Record<string, boolean>>(initialLevelFilters);
  const [isLevelFilterOpen, setIsLevelFilterOpen] = useState(false);

  const [topicGroupFilters, setTopicGroupFilters] = useState<
    Record<string, boolean>
  >({});
  const [tempTopicGroupFilters, setTempTopicGroupFilters] = useState<
    Record<string, boolean>
  >({});
  ...
  useEffect(() => {
    ...
    const initialTopicFilters = uniqueTopicGroups.reduce(
      (acc, topicGroup) => {
        acc[topicGroup] = true;
        return acc;
      },
  ...
  const uniqueTopicGroups = useMemo(
    () => [...new Set(lessons.map((l) => l.topicGroup))].sort(),
    [lessons],
  );
  ...
  const handleCreateReviewTest = async () => {
    ...
    if (completedLessons.length === 0) {
      toast({
        ...
        description:
          "You must complete at least one lesson to create a review test.",
      });
      return;
    }
    ...
    try {
      const vocabulary = completedLessons.flatMap((l) => {
        ...
        try {
          return JSON.parse(vocabContent.value).map(
            (v: { word: string; definition: string }) => ({
              term: v.word,
      ...
      const passages = completedLessons
        .map((l) => {
      ...
      if (vocabulary.length === 0 && passages.length === 0) {
        toast({
          ...
          description:
            "Your completed lessons do not have enough vocabulary or reading passages to generate a test.",
        });
      ...
      toast({
        ...
        description: "Could not create a review test. Please try again.",
      });
    } finally {
      setIsGeneratingTest(false);
    }
    ...
    return (
      ...
      >
        {Object.entries(groupedLessons).map(([topicGroup, groupLessons]) => {
          const stats = groupLessons.reduce(
            (acc, lesson) => {
                ...
                <TableCell>
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger>
                        <StatusIcon
                          className={cn("h-5 w-5", statusClassName)}
                        />
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{tooltip}</p>
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              ...
              <CardDescription>
                Search, filter, and select a lesson to start practicing.
              </CardDescription>
            </div>
            <div className="flex items-center gap-2">
              <Button
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                <DropdownMenuContent
                  ...
                  onEscapeKeyDown={() => cancelTopicGroupFilters()}
                >
                  <DropdownMenuLabel>Show Groups</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem
                    onSelect={() => handleSelectAllTopics(true)}
                  >
                    <Check className="mr-2 h-4 w-4" />
                    Select All
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onSelect={() => handleSelectAllTopics(false)}
                  >
                  ...
                  {uniqueTopicGroups.map((topicGroup) => (
                    <DropdownMenuCheckboxItem
                      ...
                      onCheckedChange={() =>
                        handleTempTopicGroupFilterChange(topicGroup)
                      }
                      onSelect={(e) => e.preventDefault()}
                    >
                      {topicGroup}
                  ...
                  <div className="flex justify-end gap-2 p-2">
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={cancelTopicGroupFilters}
                    >
                      Cancel
                    </Button>
                    <Button size="sm" onClick={applyTopicGroupFilters}>
                      Apply
                    </Button>
                  </div>
                </DropdownMenuContent>
              ...
              >
                ...
                <DropdownMenuContent
                  onPointerDownOutside={(e) => e.preventDefault()}
                  onEscapeKeyDown={() => cancelLevelFilters()}
                >
                  ...
                  <div className="flex justify-end gap-2 p-2">
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={cancelLevelFilters}
                    >
                      Cancel
                    </Button>
                    <Button size="sm" onClick={applyLevelFilters}>
                      Apply
                    </Button>
                  </div>
                </DropdownMenuContent>
              ...
              >
                ...
                <DropdownMenuContent
                  onPointerDownOutside={(e) => e.preventDefault()}
                  onEscapeKeyDown={() => cancelSkillFilters()}
                >
                  ...
                  {Object.keys(skillFilters).map((skill) => (
                    <DropdownMenuCheckboxItem
                      ...
                      onCheckedChange={() =>
                        handleTempSkillFilterChange(skill as Skill)
                      }
                      onSelect={(e) => e.preventDefault()}
                    >
                      {skill}
                  ...
                  <div className="flex justify-end gap-2 p-2">
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={cancelSkillFilters}
                    >
                      Cancel
                    </Button>
                    <Button size="sm" onClick={applySkillFilters}>
                      Apply
                    </Button>
                  </div>
                </DropdownMenuContent>
