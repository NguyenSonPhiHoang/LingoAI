...
const MarkdownToolbar: FC<{
  ...
  const applyFormat = (
    formatType: "bold" | "italic" | "heading" | "ul" | "ol"
  ) => {
...
const AddNoteDialog: FC<{
    ...
    try {
      ...
      if (!clipboard) {
        toast({
          ...
          description:
            "This browser doesn't expose clipboard read. Click the text box and press Ctrl+V to paste.",
        });
        return;
      }

      if (!clipboard.read) {
        // Fallback: try text-only API when available.
        ...
        toast({
          variant: "destructive",
          title: "Clipboard not available",
          description: "Click the text box and press Ctrl+V to paste.",
        });
        return;
      }
      ...
      toast({
        ...
        description: "No text or image found on the clipboard.",
      });
    } catch (err) {
      console.error("Failed to read clipboard contents: ", err);
      // Fallback to text-only readText (more widely allowed)
      try {
        const clipboard = navigator?.clipboard;
        if (clipboard?.readText) {
          const text = await clipboard.readText();
          if (text) {
            ...
            toast({
              title: "Pasted",
              description: "Text was pasted from clipboard.",
            });
            return;
          }
        }
      } catch (err2) {
        console.error("Failed to read clipboard text: ", err2);
      }
      ...
      toast({
        ...
        title: "Paste Failed",
        description:
          "Browser blocked clipboard read. Click the text box and press Ctrl+V to paste instead.",
      });
    }
  };

  const handlePasteEvent: React.ClipboardEventHandler<HTMLTextAreaElement> = (
    e
  ) => {
    ...
    try {
      ...
      if (result.text) {
        ...
        toast({
          ...
          description:
            "The extracted text has been placed in the text area for you to save.",
        });
      } else {
        toast({
          variant: "destructive",
          title: "No Text Found",
          description: "AI could not find any text in the image.",
        });
      }
    } catch (error) {
  ...
  const handleSaveNote = async (
    ...
    titleToSave: string,
    type: "markdown" | "image" = "markdown"
  ) => {
    if (!contentToSave || !titleToSave) {
      toast({
        variant: "destructive",
        title: "Missing Content",
        description: "Please provide a title and content for the note.",
      });
      return;
    }
    setIsSaving(true);
    try {
      const newNote = await addContentToDocument(
        docId,
        titleToSave,
        contentToSave,
        type
  ...
  const handleSaveCanvas = async () => {
    ...
    try {
      ...
      handleSaveNote(
        dataUrl,
        `Handwritten Note - ${format(new Date(), "PP")}`,
        "image"
      );
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent
        className={cn(
          "sm:max-w-2xl",
          isCanvasFullscreen && "w-screen h-screen max-w-full p-0 flex flex-col"
        )}
      >
        <div className={cn("space-y-4", isCanvasFullscreen && "hidden")}>
          <DialogHeader>
            <DialogTitle>Add a New Note</DialogTitle>
            <DialogDescription>
              Write with Markdown, paste from clipboard, or use a stylus to
              write by hand.
            </DialogDescription>
          </DialogHeader>
        </div>
        <Tabs
          ...
          className={cn(
            "flex flex-col",
            isCanvasFullscreen ? "flex-1" : "space-y-4"
          )}
        >
          <TabsList
            className={cn(
          ...
          >
            ...
            <TabsTrigger value="paste" onClick={handlePasteFromClipboard}>
              Paste
            </TabsTrigger>
          ...
          <TabsContent value="write" className="space-y-4 pt-4">
            ...
            <DialogFooter>
              <Button
                className="w-full"
                onClick={() =>
                  handleSaveNote(noteContent, noteTitle, "markdown")
                }
                disabled={isSaving}
              >
            ...
            {clipboardImage ? (
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    <MarkdownToolbar
                      textareaRef={pastedTextareaRef}
                      onContentChange={setClipboardText}
                    />
                    <Textarea
                      ...
                      rows={10}
                      placeholder="Click here and press Ctrl+V to paste text or an image. (Some browsers block automatic clipboard access.)"
                    />
                  </div>
                </div>
                <DialogFooter>
                  <Button
                    className="w-full"
                    onClick={() =>
                      handleSaveNote(clipboardText, noteTitle, "markdown")
                    }
                    disabled={isSaving || !clipboardText}
                  >
                    {isSaving ? (
                      <Loader2 className="mr-2 animate-spin" />
                    ) : null}
          ...
          <TabsContent
            ...
            className={cn(
              "pt-4 flex flex-col",
              isCanvasFullscreen ? "flex-1 h-full" : "space-y-4"
            )}
          >
            <div
              className={cn(
                "flex flex-wrap items-center gap-4",
                isCanvasFullscreen && "p-4 border-b"
              )}
            >
            ...
            <div
              className={cn(
                ...
                isCanvasFullscreen && "flex-1 border-0 rounded-none"
              )}
            >
              <ReactSketchCanvas
                ref={canvasRef}
            ...
            <DialogFooter className={cn(isCanvasFullscreen && "p-4 border-t")}>
              <Button
                className="w-full"
