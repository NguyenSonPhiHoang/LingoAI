...
import { z } from "zod";
import { suggestPersonalizedLessons } from "@/ai/flows/suggest-personalized-lessons";
...
type SuggestPersonalizedLessonsInput = Parameters<
  typeof suggestPersonalizedLessons
>[0];
...
const AiSuggester: FC<AiSuggesterProps> = ({
  setActiveViewState,
  recommendedLevel,
}) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const newLessonPromises = result.lessonSuggestions.map((suggestion) =>
        addLesson(userId, suggestion, values.learningGoals),
      );
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <Card className="bg-primary/5 border-primary/20">
        <CardHeader>
          ...
          <CardDescription>
            Take our quick placement test to get an accurate recommendation.
          </CardDescription>
        </CardHeader>
        <CardFooter>
        ...
        <CardHeader>
          ...
          {recommendedLevel && (
            <div className="!mt-4 p-3 rounded-md bg-green-50 border border-green-200 text-green-800 flex items-center gap-2">
              ...
              <p className="text-sm">
                We've pre-selected a level for you based on your test results.
              </p>
            </div>
          )}
        </CardHeader>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      ...
                      <SelectContent>
                        {Object.entries(levelMapping).map(
                          ([key, { label }]) => (
