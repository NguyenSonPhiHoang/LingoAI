...
const AddWordDialog: FC<AddWordDialogProps> = ({ setWords, trigger }) => {
  const { user } = useAuth();
  ...
  const handleExtractFromImage = async () => {
    ...
    setExtractedWords([]);
    try {
      const result = await extractVocabularyFromFile({
        imageDataUri: clipboardImage,
      });
      if (result.vocabulary.length === 0) {
        toast({
          title: "No Words Found",
          description: "The AI could not find any vocabulary in the image.",
        });
      } else {
      ...
      toast({
        ...
        description: "Could not extract words from the image.",
      });
    } finally {
      setIsExtracting(false);
    }
  ...
  const handleSaveImportedWords = async (wordsToSave: VocabularyEntry[]) => {
    ...
    try {
      ...
      toast({
        title: "Success!",
        description: `${newWords.length} words were imported.`,
      });
      handleCloseDialog();
    } catch (error) {
      ...
      toast({
        variant: "destructive",
        title: "Save Failed",
        description: "Could not save the imported words.",
      });
    } finally {
      setIsSaving(false);
    }
  ...
  const handleSaveWord = async () => {
    ...
    try {
      ...
      const savedCombinedVocabulary = await addWordToVocabulary(
        user.uid,
        fullWordData as VocabularyEntry,
      );
      ...
      setWords((prevWords) => {
        ...
        wordsMap.set(
          savedCombinedVocabulary.userVocabularyId,
          savedCombinedVocabulary,
        );

        // Convert back to an array and sort by creation date to keep the list consistent.
        return Array.from(wordsMap.values()).sort(
          (a, b) => b.createdAt.toMillis() - a.createdAt.toMillis(),
        );
      });
      ...
      toast({
        title: "Success",
        description: `"${savedCombinedVocabulary.term}" saved to your list.`,
      });
    } catch (error) {
  ...
  const renderMainContent = () => {
    ...
    return (
      <div className="space-y-4">
        <DialogDescription>
          Enter a word/phrase, or copy text/image before opening. The tool will
          automatically paste it for you.
        </DialogDescription>
          ...
          <div className="space-y-4 rounded-lg border bg-muted/50 p-4 max-h-[300px] overflow-y-auto">
            {isGenerating ? (
              ...
              <div className="space-y-4 text-sm">
                ...
                {generatedDetails.synonyms &&
                  generatedDetails.synonyms.length > 0 && (
                    <div>
                      <strong className="text-muted-foreground">
                        Synonyms:
                      </strong>
                      <div className="flex flex-wrap gap-2 mt-1">
                        {generatedDetails.synonyms.map((s, i) => (
                ...
                {generatedDetails.antonyms &&
                  generatedDetails.antonyms.length > 0 && (
                    <div>
                      <strong className="text-muted-foreground">
                        Antonyms:
                      </strong>
                      <div className="flex flex-wrap gap-2 mt-1">
                        {generatedDetails.antonyms.map((a, i) => (
                    ...
                    <div className="flex flex-wrap gap-4 mt-1">
                      <div>
                        ...
                        <Badge variant="secondary">
                          {generatedDetails.irregularForms.v1}
                        </Badge>
                      </div>
                      <div>
                        V2:{" "}
                        <Badge variant="secondary">
                          {generatedDetails.irregularForms.v2}
                        </Badge>
                      </div>
                      <div>
                        V3:{" "}
                        <Badge variant="secondary">
                          {generatedDetails.irregularForms.v3}
                        </Badge>
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              <div className="relative border-2 border-dashed rounded-lg p-2">
                ...
                <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
                  <span className="text-white font-semibold bg-black/50 px-2 py-1 rounded">
