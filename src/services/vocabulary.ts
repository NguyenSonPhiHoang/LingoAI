...
export const addWordToVocabulary = async (
  ...
  if (!firebaseEnabled) {
    return {
      id: `word_${Date.now()}`,
      userVocabularyId: `user_vocab_${Date.now()}`,
      term: fullWordData.term,
      term_normalized: fullWordData.term.toLowerCase(),
    ...
    return words.map(
      (word, idx) =>
        ({
          ...
          userVocabularyId: `user_vocab_${Date.now()}_${idx}`,
          term_normalized: word.term.toLowerCase(),
