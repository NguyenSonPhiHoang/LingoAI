...
export const addResource = async (payload: Omit<LearningResource, 'id' | 'createdAt' | 'ratings' | 'ratingCount' | 'averageRating'>): Promise<LearningResource> => {
...
export const rateResource = async (resourceId: string, rating: number): Promise<{averageRating: number, ratingCount: number}> => {
    const user = auth.currentUser;
