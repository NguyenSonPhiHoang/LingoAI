...
const StorybookDetailPage: FC = () => {
  ...
  const playbackHook = useAudioPlayback({
      ...
      try {
        ...
        if (!fetchedStorybook) {
          toast({
            ...
            title: "Not Found",
            description:
              "This storybook does not exist or you do not have permission to view it.",
          });
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Could not fetch the storybook.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleAddWord = async (vocabItem: {
    word: string;
    definition: string;
    partOfSpeech: string;
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Could not save the word to your vocabulary.",
      });
    } finally {
  ...
  const handleStatusChange = async (newStatus: StorybookStatus) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Could not update storybook status.",
      });
    }
  };
  ...
  if (!storybook) {
    return (
      <div className="text-center">
        ...
        <Button onClick={() => router.push("/storybook")} className="mt-4">
          Back to Library
        </Button>
      </div>
    );
  ...
  const currentStatusInfo =
    statusOptions.find((s) => s.value === storybook.status) || statusOptions[0];
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <DropdownMenuContent>
                  ...
                  {statusOptions.map((option) => (
                    <DropdownMenuItem
                      ...
                      disabled={storybook.status === option.value}
                    >
              ...
              <Button
                variant="outline"
                onClick={() => playAndCacheStoryAudio("title", storybook.title)}
                disabled={playbackHook.isLoadingAudio[titleAudioKey]}
              >
                {playbackHook.isLoadingAudio[titleAudioKey] ? (
              ...
              {storybook.format === "interspersed" &&
                storybook.interspersedStory && (
                  ...
                  >
                    {playbackHook.isLoadingAudio[englishAudioKey] ? (
                      <Loader2 className="animate-spin h-4 w-4" />
                    ) : (
            ...
            <CardContent>
              <ul className="space-y-4 text-sm columns-1 md:columns-2">
                {storybook.keyVocabulary.map((v, i) => {
                  ...
                  const isVocabAudioLoading =
                    playbackHook.isLoadingAudio[`${vocabAudioKey}-term`] ||
                    playbackHook.isLoadingAudio[`${vocabAudioKey}-sentence`];
                  const isDefinitionAudioLoading =
                    playbackHook.isLoadingAudio[definitionAudioKey];
                  const isTranslating =
                  ...
                  return (
                    <li
                      key={i}
                      className="flex flex-col gap-2 break-inside-avoid-column p-3 rounded-md bg-background border"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              <div>
                ...
                <article className="prose prose-sm dark:prose-invert max-w-none [&_p]:my-1 [&_h1]:mt-2 [&_h1]:mb-1 [&_h2]:mt-2 [&_h2]:mb-1">
                  <InteractiveText
                    ...
                    activePlaybackKey={englishAudioKey}
                  />
                </article>
              </div>
              <div className="border-t md:border-l md:border-t-0 md:pl-8 pt-4 md:pt-0">
                <div className="flex justify-between items-center mb-2">
                  ...
                  >
                    {playbackHook.isLoadingAudio[vietnameseAudioKey] ? (
                      <Loader2 className="animate-spin h-5 w-5" />
                    ) : (
                      <Volume2 className="h-5 w-5" />
                    )}
                ...
                <article className="prose prose-sm dark:prose-invert max-w-none [&_p]:my-1 [&_h1]:mt-2 [&_h1]:mb-1 [&_h2]:mt-2 [&_h2]:mb-1">
                  <InteractiveText
                    ...
                    vocabulary={[]}
                    playbackHook={playbackHook}
                    activePlaybackKey={vietnameseAudioKey}
                  />
                </article>
              </div>
            </div>
