...
const StorybookCard: FC<{
  story: Storybook;
  onStoryDeleted: (id: string) => void;
}> = ({ story, onStoryDeleted }) => {
    ...
    <Card className="flex flex-col hover:shadow-lg transition-shadow">
      <CardHeader className={cn("relative p-3", formatClass)}>
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className="capitalize bg-background/50">
              {story.format === "interspersed" ? "Interspersed" : "Bilingual"}
            </Badge>
          </div>
          <DropdownMenu>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This will permanently delete "{story.title}".
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger className="mt-1">
                <StatusIcon
                  className={cn("h-5 w-5 flex-shrink-0", statusClassName)}
                />
              </TooltipTrigger>
              <TooltipContent>
      ...
      <CardFooter className="p-3">
        <Button
          className="w-full"
          variant="outline"
          onClick={() => router.push(`/storybook/${story.id}`)}
        >
          Read Story
        </Button>
      </CardFooter>
...
const StorybookLibraryPage: FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchStorybooks = async () => {
      setIsLoading(true);
      try {
        const fetchedStorybooks = await getStorybooks(userId);
        setStorybooks(fetchedStorybooks);
      } catch (error) {
        toast({
          variant: "destructive",
          title: "Error",
          description: "Could not fetch your storybooks.",
        });
      } finally {
        setIsLoading(false);
      }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <CardTitle className="flex items-center gap-2">
                <BookImage /> My Storybooks
              </CardTitle>
              <CardDescription>
                Your personal library of AI-generated stories. Read them again
      ...
      {filteredStorybooks.length > 0 ? (
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent>
                            <DropdownMenuItem
                              onClick={() =>
                                router.push(`/storybook/${story.id}`)
                              }
                            >
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                                  <AlertDialogAction
                                    onClick={() => handleDeleteFromList(story)}
                                  >
                                    Delete
                                  </AlertDialogAction>
                                </AlertDialogFooter>
        ...
        <div className="text-center py-16 border-2 border-dashed rounded-lg">
          ...
          <h3 className="mt-4 text-lg font-medium">
            Your Storybook Library is Empty
          </h3>
          <p className="mt-1 text-sm text-muted-foreground">
            {searchTerm
              ? "No stories match your search."
              : "Click 'Generate New Story' to create your first one."}
          </p>
        </div>
      )}
    </div>
