    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This will delete the document and all its associated notes. This
            action cannot be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>Cancel</AlertDialogCancel>
          <AlertDialogAction onClick={() => onDocumentDeleted(doc)}>
            Delete
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
...
const LibraryPage: FC = () => {
  ...
  const [documents, setDocuments] = useState<
    Record<LibrarySkill, LibraryDocument[]>
  >({
    Reading: [],
    Writing: [],
    Listening: [],
    ...
    getDocumentsGroupedBySkill(user.uid)
      ...
      .catch((err) => {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Could not fetch your library.",
        });
      })
  ...
  const handleDocumentUpdated = (updatedDoc: LibraryDocument) => {
    setDocuments((prev) => {
      ...
      if (oldDoc.skill !== updatedDoc.skill) {
        newGroupedDocs[oldDoc.skill] = newGroupedDocs[oldDoc.skill].filter(
          (d) => d.id !== updatedDoc.id,
        );
        newGroupedDocs[updatedDoc.skill] = [
          updatedDoc,
          ...newGroupedDocs[updatedDoc.skill],
        ];
      } else {
        newGroupedDocs[updatedDoc.skill] = newGroupedDocs[updatedDoc.skill].map(
          (d) => (d.id === updatedDoc.id ? updatedDoc : d),
        );
      }
  ...
  const handleDocumentDeleted = (deletedDoc: LibraryDocument) => {
    deleteDocument(deletedDoc.id)
      .then(() => {
        setDocuments((prev) => ({
          ...prev,
          [deletedDoc.skill]: prev[deletedDoc.skill].filter(
            (d) => d.id !== deletedDoc.id,
          ),
        }));
  ...
  const filteredDocuments = useMemo(() => {
    ...
    for (const skill in documents) {
      filtered[skill as LibrarySkill] = documents[skill as LibrarySkill].filter(
        (doc) =>
  ...
  const totalDocs = Object.values(filteredDocuments).reduce(
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Organize your learning resources by skill. Add links and attach
                notes.
              </CardDescription>
            </div>
                ...
                {viewMode === "grid" ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {skillDocs.map((doc) => (
                      <Card
                        key={doc.id}
                        className="flex flex-col hover:bg-muted/50 transition-colors"
                      >
                        <CardHeader className="flex-grow">
                          <a
                            ...
                            className="font-semibold hover:underline block flex items-start gap-1.5 group"
                          >
                        ...
                        <TableRow key={doc.id}>
                          <TableCell>
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="font-medium hover:underline flex items-center gap-2 group max-w-md"
                            >
