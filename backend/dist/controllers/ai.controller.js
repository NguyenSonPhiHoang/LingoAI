...
function getClientForKey(apiKey) {
    ...
    if (cached)
        return cached;
    const created = new generative_ai_1.GoogleGenerativeAI(trimmed);
...
async function getApiKeyForUser(userId) {
    ...
    if (typeof userKey === "string" && userKey.trim())
        return userKey;
    const serverKey = config_1.config.geminiApiKey;
    if (typeof serverKey === "string" && serverKey.trim())
        return serverKey;
    throw new Error("Gemini API key not configured. Add geminiApiKey in Settings or set GEMINI_API_KEY on the backend.");
}
...
class AiController {
    static async chat(req, res) {
        ...
        try {
            ...
            const isConfigError = msg.toLowerCase().includes("api key") ||
                msg.toLowerCase().includes("not configured");
            return res.status(isConfigError ? 400 : 500).json({ error: msg });
        }
    }
    static async replySuggestions(req, res) {
        const userId = getUserId(req);
        if (!userId)
            return res.status(401).json({ error: "unauthorized" });
        ...
        const max = typeof maxSuggestions === "number" && maxSuggestions > 0
        ...
        try {
            ...
            const model = client.getGenerativeModel({
                model: modelName,
                systemInstruction: "You generate short, natural English reply suggestions for the user. Return ONLY a JSON array of strings, no markdown, no explanation.",
            });
    ...
    static async translate(req, res) {
        ...
        const target = typeof targetLanguage === "string" && targetLanguage.trim()
            ? targetLanguage.trim()
            : "vi";
        ...
        try {
            ...
            const model = client.getGenerativeModel({
                model: modelName,
                systemInstruction: "You are a translation engine. Translate faithfully, preserve formatting (lists, line breaks). Return only the translated text, no extra commentary.",
            });
