  ...
  try {
    ...
    if (doc.TocJson) {
      try {
        ...
        if (toc.Writing) {
          console.log('Writing skill config:', JSON.stringify(toc.Writing, null, 2));
        }
      } catch (e) {
    ...
    if (itemsResult.recordset.length > 0) {
      ...
      const bySkill = itemsResult.recordset.reduce((acc, item) => {
      ...
      console.log('\n🔸 First 5 items:');
      itemsResult.recordset.slice(0, 5).forEach((item, idx) => {
        console.log(`  ${idx + 1}. Skill: ${item.Skill}, Part: ${item.Part}, SectionKey: ${item.SectionKey}`);
        console.log(`     Prompt preview: ${(item.Prompt || '').substring(0, 100)}...`);
      });
    }
    
    // Check if there are any Writing items specifically
    const writingItemsResult = await pool.request()
      .input('DocumentId', documentId)
      .query(`
