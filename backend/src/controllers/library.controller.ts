...
export class LibraryController {
  ...
  static async fetchMyDocumentContent(req: AuthRequest, res: Response) {
    const userId = getUserId(req);
    if (!userId) return res.status(401).json({ error: "unauthorized" });
    ...
    // Get the document to verify ownership and get URL
    const doc = await LibraryRepository.getDocumentById(docId, userId);
    if (!doc) return res.status(404).json({ error: "document not found" });
    if (!doc.url) return res.status(400).json({ error: "document has no URL" });
    ...
    try {
      ...
      if (!response.ok) {
        return res.json({ 
          content: "Unable to fetch content from this URL. The resource may be unavailable or require authentication.",
      ...
      if (contentType.includes("text/html")) {
        ...
        // Simple HTML to text conversion - remove scripts, styles, and extract text
        ...
        // Limit content length to avoid overwhelming the UI
        const truncatedContent = textContent.length > 5000 
          ? textContent.substring(0, 5000) + "...\n\n[Content truncated. View full article at the original URL]"
          : textContent;
          
        return res.json({ content: truncatedContent });
        ...
        return res.json({ 
          content: "This document contains non-text content that cannot be displayed here. Please visit the original URL to view the content.",
        });
      }
    } catch (fetchError: any) {
      console.error("Error fetching document content:", fetchError);
      return res.json({ 
        content: "Unable to fetch content from this URL. The resource may be behind a firewall, require authentication, or be temporarily unavailable. Please visit the original URL to view the content.",
      });
    }
  }
}
